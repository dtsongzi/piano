...
function App() {
  ...
  // 播放音符声音
  const playNote = useCallback((keyName: string) => {
    if (!audioContextRef.current) {
      audioContextRef.current = new (window.AudioContext || (window as any).webkitAudioContext)()
    }
    
    const audioContext = audioContextRef.current
    let noteToPlay = keyName
    
    // 在非严格模式下，将按键转换为C4八度处理
    if (!strictMode) {
      // 提取音符基础部分（保留升降号，去除八度编号）
      const noteBase = keyName.replace(/\d+$/, '')
      // 固定使用4八度
      noteToPlay = noteBase + '4'
    }
    
    const frequency = pitchFrequencies[noteToPlay]
    if (!frequency) return
  ...
  }, [currentNote, strictMode, generateRandomNote, noteStartTime, playNote])
